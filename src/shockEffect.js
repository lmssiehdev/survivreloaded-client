...
var shockEffect = function () {
    /**
     * Constructor
     * @param {boolean} runningOnClient Indicates if the effect will run on the client or the server 
     */
    function shockEffect(runningOnClient, params) {
        _classCallCheck(this, shockEffect);

        this.id = 6;
        this.name = "m_shockEffect";
        ...
        this.type = StatusEffectAttackType.Shock;
    ...
    _createClass(shockEffect, [{
        ...
        value: function update(player, dt, damageList) {
            ...
            if (!this.runningOnClient) {
                player.setExtraDamage(this.effectData.damagePercentage);

                if (this.ticker <= 0) {
                    player.setExtraDamage(0.0);
                    this.dirty = true;
                }
                var time = this.effectData.time;
